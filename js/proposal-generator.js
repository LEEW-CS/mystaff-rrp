...
async function generateProposal() {
    ...
    const clientCompany  = clientLast ? clientLast : clientFirst;
...
async function buildProposalPPTX(d) {
    ...
    // ── SLIDE 4: Pricing ────────────────────────────────────────
    {
        ...
        boxes.forEach(box => {
            ...
            const bodyRuns = box.lines.map((l, i) => ({ text: l, options: { breakLine: i < box.lines.length - 1 } }));
            s.addText(bodyRuns, { x: box.x + 0.15, y: boxY + 0.52, w: 3.8, h: boxH - 0.65, fontSize: 10, color: CS_DARK, fontFace: 'Arial', align: 'left', valign: 'top', lineSpacingMultiple: 1.25 });
        });
    ...
    // ── SLIDE 7: Work Options ───────────────────────────────────
    {
        ...
        s.addText('CLOUDSTAFF YOUR WORKFORCE IN THE CLOUD',
            { x: 7.0, y: 0.2, w: 6.0, h: 0.3, fontSize: 9, color: 'AAAAAA', fontFace: 'Arial', align: 'right', charSpacing: 1.5 });
    ...
    // ── SLIDE 9: Experience & Credentials ───────────────────────
    {
        ...
        s.addText(creds.map((c, i) => ({ text: c, options: { breakLine: i < creds.length - 1 } })),
